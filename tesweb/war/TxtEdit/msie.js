...
CodePress = {
    ...
    //scckobe：2009-11-04
    //自动添加分号
    autoSemi: function(evt) {
        var lastLetter = this.getLastLetter();
        //空行不做处理
        if (lastLetter == "")
            return;
        //如果没有分号符，则自动添加分号符
    ...
    getLastLetter: function() {
        ...
        var wordLine = rangeAndCaret[0].substring(rangeAndCaret[1] - 40, rangeAndCaret[1]);
        wordLine = wordLine.replace(/[\s\n\r\);]/g, '\n').split('\n');
        var letters = wordLine[wordLine.length - 1];
        if (letters == "")
            return "";
        else {
            //如果注释行，也不做处理
            if (letters.length >= 2) {
                if (letters.charAt(0) == "/" && (letters.charAt(1) == "/" || letters.charAt(1) == "*"))
                    return "";
            }
